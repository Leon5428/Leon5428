...
内容未改变的输出文件直接跳过，避免重复替换正在预览的图片。文件内容改变时仍使用
临时文件进行原子替换；遇到拒绝访问时最多尝试 5 次，累计等待约 1.5 秒。
若仍失败，保留该文件的旧版本并报告路径。关闭占用该文件的程序，检查只读属性
及目录写入权限后重试；不需要删除整个 `Output` 或默认以管理员身份运行。
